...
public class TrayManagerPlugin: NSObject, FlutterPlugin, NSMenuDelegate {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "popUpContextMenu":
            ...
            break
        case "cancelContextMenu":
            cancelContextMenu(call, result: result)
            break
        default:
    ...
    public func setContextMenu(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        // If the new menu has the same shape as the currently installed one,
        // mutate the existing NSMenuItems in place. This keeps the visible
        // menu live-updated (delay labels refresh while the menu is open) and
        // preserves NSMenuItem tags so clicks on the displayed menu still map
        // to the right Dart MenuItem ids.
        if let existing = trayMenu, existing.tryUpdate(menuArgs) {
            result(true)
            return
        }
    ...
    public func cancelContextMenu(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        trayMenu?.cancelTracking()
        result(true)
    }
