...
class TrayManager {
  ...
  ///
  /// If the new menu has the same shape as the previous one (same item count
  /// and types at each level), label/checked/onClick/etc. are mutated onto the
  /// existing [MenuItem]s in place. This keeps generated ids stable across
  /// rebuilds, so clicks on items in an already-displayed native menu still
  /// resolve correctly after the menu data is refreshed in the background.
  Future<void> setContextMenu(Menu menu) async {
    final existing = _menu;
    if (existing != null && _menusHaveSameShape(existing, menu)) {
      _mergeMenu(existing, menu);
    } else {
      _menu = menu;
    }
    final Map<String, dynamic> arguments = {
  ...
  bool _menusHaveSameShape(Menu a, Menu b) {
    final aItems = a.items ?? const <MenuItem>[];
    final bItems = b.items ?? const <MenuItem>[];
    if (aItems.length != bItems.length) return false;
    for (int i = 0; i < aItems.length; i++) {
      if (aItems[i].type != bItems[i].type) return false;
      final aSub = aItems[i].submenu;
      final bSub = bItems[i].submenu;
      if ((aSub == null) != (bSub == null)) return false;
      if (aSub != null && bSub != null && !_menusHaveSameShape(aSub, bSub)) {
        return false;
      }
    }
    return true;
  ...
  void _mergeMenu(Menu existing, Menu next) {
    final existingItems = existing.items!;
    final nextItems = next.items!;
    for (int i = 0; i < existingItems.length; i++) {
      final old = existingItems[i];
      final updated = nextItems[i];
      old.key = updated.key;
      old.label = updated.label;
      old.sublabel = updated.sublabel;
      old.toolTip = updated.toolTip;
      old.icon = updated.icon;
      old.checked = updated.checked;
      old.disabled = updated.disabled;
      old.onClick = updated.onClick;
      old.onHighlight = updated.onHighlight;
      old.onLoseHighlight = updated.onLoseHighlight;
      if (old.submenu != null && updated.submenu != null) {
        _mergeMenu(old.submenu!, updated.submenu!);
      }
    }
  }
  ...
  /// Cancels tracking of the currently open context menu.
  Future<void> cancelContextMenu() async {
    await _channel.invokeMethod('cancelContextMenu');
  }
